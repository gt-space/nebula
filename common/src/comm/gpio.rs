...
impl Gpio {
  pub fn open_controller(controller_index: usize) -> Gpio {
    ...
    // Immediately lock base
    let base_ptr = *base.lock().unwrap();

    if base_ptr.is_null() {
      panic!("Cannot map GPIO");
    } else if base_ptr != GPIO_BASE_REGISTERS[controller_index] as *mut c_void {
    ...
    let direction = Mutex::new(unsafe { 
      base_ptr.offset(GPIO_OE_REGISTER) as * mut u32
    });

    let dataout = Mutex::new(unsafe {
      base_ptr.offset(GPIO_DATAOUT_REGISTER) as *mut u32
    });

    let datain = unsafe {
      base_ptr.offset(GPIO_DATAIN_REGISTER) as *const u32
    };
